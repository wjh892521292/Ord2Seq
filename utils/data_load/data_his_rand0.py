...
class MyDataset(data_utils.Dataset):
    ...
    def __init__(self, img_root, data_root, dataset, transform=None, fold=3):
        ...
        f = open('/data2/chengyi/dataset/ord_reg/historical/data_265/new_rand0/{}_{}.csv'.format(dataset, fold), "r")
        ...
        # f = open('wodewenjian.csv','r')
        # reader = csv.reader(f)
        # next(reader)
        # tmp = []
        # for row in reader:
        #     # row: [lat, lon, rsrp]
        #     station, lat, lon, rsrp = row   
        #     tmp.append([lat, lon, rsrp])
        # tmp.sort(key = lambda x:x[2])
        # # 以rsrp来排序
        # tmp = tmp[:3]
        # lats = [x[0] for x in tmp] # [lat1, lat2, lat3]
        # lons = [x[1] for x in tmp]
        # process(lats, lons)
...
def make_data_set():
    for i in range(10):
        ...
        column = ['name', 'label']
        test = pd.DataFrame(columns=column, data=train)
        test.to_csv('/data2/chengyi/dataset/ord_reg/historical/data_265/new_rand1/train_{}.csv'.format(i),
                    encoding='gbk')

        column = ['name', 'label']
        test = pd.DataFrame(columns=column, data=val)
        test.to_csv('/data2/chengyi/dataset/ord_reg/historical/data_265/new_rand1/valid_{}.csv'.format(i),
                    encoding='gbk')
